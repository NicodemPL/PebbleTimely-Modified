...
void weather_layer_update_callback(Layer *me, GContext* ctx) {
  ...
  graphics_draw_text(ctx, cond_current, climacons, GRect(0,-8,36,36), GTextOverflowModeWordWrap, GTextAlignmentCenter, NULL);
  graphics_draw_text(ctx, temp_current, fonts_get_system_font(FONT_KEY_GOTHIC_14), GRect(34,2,26,20), GTextOverflowModeWordWrap, GTextAlignmentLeft, NULL);
#else
...
#if HIDE_BATTERY_DISPLAY
  // Position date at top-right, leaving room for weather icon on left (5 pixels higher total)
  layer_set_frame( text_layer_get_layer(date_layer), GRect(40, REL_CLOCK_DATE_TOP + date_vert_offset - 5, 102, REL_CLOCK_DATE_HEIGHT) );
#else
...
void position_time_layer() {
  ...
  layer_set_frame( weather_layer, GRect(0, 0, 62, 28) );
#else
